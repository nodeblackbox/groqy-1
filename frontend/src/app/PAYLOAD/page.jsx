...
import GlobalVariablesManager from './GlobalVariableManager/page';



// Custom UI Components
const Button = ({ onClick, children, variant = 'primary', size = 'md', className = '', ...props }) => {
    const baseClasses = 'flex items-center justify-center space-x-1 px-4 py-2 rounded transition-colors';
    const variants = {
...
const Input = ({ value, onChange, placeholder, className = '', icon, ...props }) => (
    <div className={`relative ${className}`}>
        ...
        <input
            ...
            className={`w-full px-3 py-2 pl-${icon ? '10' : '3'} bg-gray-800 border border-gray-600 rounded text-white focus:outline-none focus:ring-2 focus:ring-blue-500`}
            {...props}
        />
    </div>
);
...
const TextArea = ({ value, onChange, placeholder, className = '', ...props }) => (
    <textarea
        ...
        onChange={onChange}
        placeholder={placeholder}
        className={`w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded text-white focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y ${className}`}
        {...props}
    />
);
...
// Main Page Component
export default function EnhancedPayloadTester() {
    // State Variables
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white p-8">
            ...
            <h1 className="text-4xl font-bold mb-8">Enhanced Payload Tester</h1>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <Button onClick={createPayload} className="w-full bg-blue-600 hover:bg-blue-700 transition-colors">
                    <Plus className="mr-2 h-4 w-4" /> New Payload
                </Button>
                <Button onClick={createRoutine} className="w-full bg-green-600 hover:bg-green-700 transition-colors">
                    <Plus className="mr-2 h-4 w-4" /> New Routine
                </Button>
                <Button onClick={() => setIsImportModalOpen(true)} className="w-full bg-purple-600 hover:bg-purple-700 transition-colors">
                    <Upload className="mr-2 h-4 w-4" /> Import Config
                </Button>
                <Button onClick={handleExportConfig} className="w-full bg-orange-600 hover:bg-orange-700 transition-colors">
                    <Download className="mr-2 h-4 w-4" /> Export Config
                </Button>
            </div>

            <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4">
                {/* Payloads and Routines Sidebar */}
                    ...
                    <div className="mb-8">
                        ...
                        <div className="mb-4 flex items-center space-x-2">
                            ...
                            <select
                                ...
                                className="px-3 py-2 bg-gray-800 border border-gray-700 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                            >
                        ...
                        <ul>
                            {filteredPayloads.map((payload, index) => (
                                <li
                                    ...
                                    className={`p-2 rounded cursor-pointer mb-2 ${activePayload && activePayload.id === payload.id ? 'bg-blue-600' : 'bg-gray-800'
                                        }`}
                    ...
                    <div>
                        ...
                        <ul>
                            {routines.map((routine) => (
                                <li
                                    key={routine.id}
                                    className={`p-2 rounded cursor-pointer mb-2 ${activeRoutine && activeRoutine.id === routine.id ? 'bg-green-600' : 'bg-gray-800'
                                        }`}
                ...
                <div className="w-full lg:w-2/4">
                    ...
                    {/* Tabs for Edit, JSON Structure, Routine Editor */}
                    <Tabs activeTab={activePayload ? 'edit' : activeRoutine ? 'routine' : 'structure'}>
                        <div className="flex space-x-2 mb-4">
                            <Button
                                variant={activePayload ? 'primary' : 'ghost'}
                                onClick={() => setActivePayload(activePayload)}
                            >
                                Edit Payload
                            </Button>
                            <Button
                            ...
                            <Button
                                ...
                                onClick={() => setActiveRoutine(activeRoutine)}
                            >
                                Routine Editor
                            </Button>
                        </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        {activePayload.subtasks.map((subtask) => (
                                            <div key={subtask.id} className="bg-gray-700 p-4 rounded mb-4">
                                                ...
                                                <select
                                                    ...
                                                    className="px-3 py-2 bg-gray-600 border border-gray-500 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 mb-2"
                                                >
                                                    <option value="GET">GET</option>
                                                    <option value="POST">POST</option>
                                                    <option value="PUT">PUT</option>
                                        ...
                                        <Button onClick={() => addSubtask(activePayload.id)}>Add Subtask</Button>
                                    </div>
                        ...
                        {!activePayload && !activeRoutine && (
                            <div className="flex flex-col">
                                ...
                                <div className="bg-gray-800 p-6 rounded-xl">
                                    ...
                                    <pre className="bg-gray-700 p-4 rounded-lg overflow-auto max-h-64">
                                ...
                                <div className="space-y-4">
                                    ...
                                    {activeRoutine.payloads.length === 0 ? (
                                        ...
                                        <ul className="space-y-2">
                                            {activeRoutine.payloads.map(payloadId => {
                                                const payload = payloads.find(p => p.id === payloadId);
                                                return payload ? (
                                                    <li key={payloadId} className="flex justify-between items-center bg-gray-700 p-2 rounded">
                                    ...
                                    <div className="mt-4">
                                        <select
                                            ...
                                            className="px-3 py-2 bg-gray-700 border border-gray-600 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"
                                            defaultValue=""
                                        >
                ...
                {/* Right Panel for Variable Outputs and Test Results */}
                        ...
                        {Object.keys(globalVariables).length === 0 ? (
                            ...
                            <ul>
                                {Object.entries(globalVariables).map(([key, value]) => (
                                    <li key={key} className="mb-2 p-2 bg-gray-700 rounded flex justify-between items-center">
                                        <span>{key}</span>
                                        <Button
                                            variant="ghost"
                                            size="xs"
                        ...
                        {Object.keys(results).length === 0 ? (
                            ...
                            <div className="max-h-64 overflow-auto">
                                {Object.entries(results).map(([id, result]) => {
                                    ...
                                    return payload ? (
                                        <div key={id} className="mb-4 p-2 bg-gray-700 rounded">
                                            <h3 className="font-semibold">{payload.name}</h3>
                                            <pre className="bg-gray-600 p-2 rounded-lg overflow-auto text-xs">
