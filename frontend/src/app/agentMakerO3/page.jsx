...
//     { id: "llama-3.1-70b-versatile", name: "Llama 3.1 70B (Preview)", contextWindow: 131072 },
//     { id: "mixtral-8x7b-32768", name: "Mixtral 8x7B", contextWindow: 32768 },
//     { id: "gemma2-9b-it", name: "Gemma 2 9B", contextWindow: 8192 },
//     { id: "gemma-7b-it", name: "Gemma 7B", contextWindow: 8192 },
//     { id: "llama3-groq-70b-8192-tool-use-preview", name: "Llama 3 70B Tool Use (Preview)", contextWindow: 8192 },
//     { id: "llama3-groq-8b-8192-tool-use-preview", name: "Llama 3 8B Tool Use (Preview)", contextWindow: 8192 },
//     { id: "llama-3.1-8b-instant", name: "Llama 3.1 8B (Preview)", contextWindow: 131072 },
//     { id: "llama-guard-3-8b", name: "Llama Guard 3 8B", contextWindow: 8192 },
//     { id: "llava-v1.5-7b-4096-preview", name: "LLaVA 1.5 7B", contextWindow: 4096 },
//     { id: "llama3-70b-8192", name: "Meta Llama 3 70B", contextWindow: 8192 },
//     { id: "llama3-8b-8192", name: "Meta Llama 3 8B", contextWindow: 8192 },
// ];
...
// // ACEAgent Class
// class ACEAgent {
//     constructor(id, name, knowledgeBase, model, contextLength, ethics, morality, splitPropensity) {
//         this.id = id;
//         this.name = name;
...
//         this.splitPropensity = splitPropensity;
//         this.layers = {
//             aspirational: {
//                 morality: "",
//                 ethics: "",
...
//     // Methods for updating layers (unchanged)
//     updateAspirationalLayer(morality, ethics, mission) {
//         this.layers.aspirational = { morality, ethics, mission };
//     }

//     updateGlobalStrategy(context, strategy) {
//         this.layers.globalStrategy = { environmentalContext: context, strategy };
//     }

//     updateAgentModel(capabilities, limitations, memory) {
//         this.layers.agentModel = { capabilities, limitations, memory };
//     }

//     updateExecutiveFunction(risks, resources, plans) {
//         this.layers.executiveFunction = { risks, resources, plans };
//     }

//     updateCognitiveControl(taskSelection, taskSwitching) {
//         this.layers.cognitiveControl = { taskSelection, taskSwitching };
//     }

//     updateTaskProsecution(success, failure, individualTasks) {
//         this.layers.taskProsecution = { success, failure, individualTasks };
//     }

//     updateInputOutput(motors, sensors, telemetry, devices, api) {
//         this.inputOutput = { motors, sensors, telemetry, devices, api };
//     }

//     async generateResponse(input, apiKey, agentFactory) {
//         try {
...
// Ethics Level: ${this.ethics}
// Morality Level: ${this.morality}
// Split Propensity: ${this.splitPropensity}

// Please respond to the user's input while considering your layers, especially your morality, ethics, and mission. Incorporate relevant knowledge base information in your response. Use markdown formatting for rich text output, including code blocks where appropriate.

// If the task involves mathematical equations, use LaTeX formatting for equations, enclosed in double dollar signs ($$).

// If the task is complex or requires multiple steps, consider splitting it into subtasks. Your split propensity of ${this.splitPropensity} indicates how likely you are to split tasks. Higher values mean you should be more inclined to split tasks.

// If you decide to split the task, format your response as follows:
// SPLIT_TASK
// Task 1: [Description of first subtask]
// Task 2: [Description of second subtask]
// ...
// END_SPLIT_TASK

// Otherwise, provide your response as normal.`;
...
//             if (result.includes("SPLIT_TASK")) {
//                 const tasks = result
//                     .split("SPLIT_TASK")[1]
//                     .split("END_SPLIT_TASK")[0]
//                     .trim()
//                     .split("\n")
//                     .filter(task => task.trim() !== "")
//                     .map(task => task.split(": ")[1]);

//                 const subAgents = await Promise.all(tasks.map(task =>
//                     agentFactory.createAgentsFromPrompt(task, this.model, this.contextLength, this.ethics, this.morality, this.splitPropensity)
//                 ));

//                 this.subAgents = subAgents.flat();
//                 return `Task split into ${tasks.length} subtasks. Created ${this.subAgents.length} sub-agents to handle them.`;
//             }
...
//     async createAgentsFromPrompt(prompt, model, contextLength, ethics, morality, splitPropensity, maxRetries = 3) {
//         let retries = 0;
...
// Create agents that align with this information and the user's prompt. Ensure each agent has a unique name and fully populated layers. The response MUST be a valid JSON array of agent objects.
...
//                 const generatedAgentsString = data.choices[0].message.content;

//                 // Attempt to parse the JSON
//                 let generatedAgents;
//                 try {
//                     generatedAgents = JSON.parse(generatedAgentsString);
//                 } catch (parseError) {
//                     console.error("JSON parsing error:", parseError);
//                     throw new Error("Invalid JSON format in the response");
//                 }

//                 // Validate the structure of the generated agents
//                 if (!Array.isArray(generatedAgents)) {
//                     throw new Error("Generated agents must be an array");
//                 }

//                 return generatedAgents.map(agentData => {
//                     const agent = new ACEAgent(generateUniqueId(), agentData.name, this.knowledgeBase, model, contextLength, ethics, morality, splitPropensity);
...
//                 });
//             } catch (error) {
//                 console.error(`Error in createAgentsFromPrompt (attempt ${retries + 1}):`, error);
//                 retries++;
//                 if (retries >= maxRetries) {
//                     throw new Error(`Failed to create agents after ${maxRetries} attempts: ${error.message}`);
//                 }
//             }
//         }
//     }
...
//     async delegateTask(task, agents, model, contextLength) {
...
// Delegate the task to the most suitable agent and provide any necessary additional instructions or data from the knowledge base. Use markdown formatting for rich text output, including code blocks where appropriate.
...
//                     <div className="mt-4">
//                         <h4 className="text-md font-semibold text-blue-300 mb-2">Sub-Agents</h4>
//                         <ul className="list-disc list-inside">
//                             {agent.subAgents.map((subAgent, subIndex) => (
//                                 <li key={subIndex} className="text-gray-300">{subAgent.name}</li>
//                             ))}
//                         </ul>
//                     </div>
//                 )}
...
// // Chat Message Component with Markdown and LaTeX support
...
//                             )
//                         },
//                         p: ({ children }) => <p>{renderLatex(Array.isArray(children) ? children.join('') : children)}</p>
//                     }}
//                 >
//                     {message}
//                 </ReactMarkdown>
...
//     const [selectedModel, setSelectedModel] = useState(SUPPORTED_MODELS.find(model => model.id === "llama-3.1-70b-versatile"));
//     const [contextLength, setContextLength] = useState(selectedModel.contextWindow);
//     const [ethics, setEthics] = useState(50);
//     const [morality, setMorality] = useState(50);
//     const [splitPropensity, setSplitPropensity] = useState(50);
...
//     const addAgent = () => {
//         const newAgent = new ACEAgent(generateUniqueId(), `Agent ${agents.length + 1}`, knowledgeBase, selectedModel.id, contextLength, ethics, morality, splitPropensity);
...
//             const generatedAgents = await agentFactory.createAgentsFromPrompt(workflow.input, selectedModel.id, contextLength, ethics, morality, splitPropensity);
...
//                 const delegation = await agentDelegator.delegateTask(currentInput, generatedAgents, selectedModel.id, contextLength);
//                 setThinkingLog((prevLog) => [...prevLog, `Task delegated: ${delegation}`]);

//                 const response = await agent.generateResponse(currentInput, apiKey, agentFactory);
...
//                     setThinkingLog((prevLog) => [...prevLog, `${agent.name} created ${agent.subAgents.length} sub-agents.`]);
//                     for (const subAgent of agent.subAgents) {
//                         const subAgentResponse = await subAgent.generateResponse(response, apiKey, agentFactory);
//                         setWorkflow(prev => ({
//                             ...prev,
//                             output: prev.output + `#### Sub-Agent: ${subAgent.name}\n**Input:**\n${response}\n\n**Output:**\n${subAgentResponse}\n\n`
//                         }));
//                     }
//                 }
...
//                 body: JSON.stringify({
//                     messages: [
//                         { role: "system", content: "You are an AI assistant helping to create an initial prompt for an ACE Workflow. Provide concise and relevant responses. Use markdown formatting for rich text output, including code blocks where appropriate. For mathematical equations, use LaTeX formatting enclosed in double dollar signs ($$)." },
...
//     const createAgentsFromChat = async () => {
//         if (!apiKey) {
//             setError("Please enter your API Key in the settings.");
//             return;
//         }
...
//         try {
//             const agentFactory = new AgentFactory(apiKey, knowledgeBase);
//             const generatedAgents = await agentFactory.createAgentsFromPrompt(suggestedPrompt, selectedModel.id, contextLength, ethics, morality, splitPropensity);

//             setAgents(generatedAgents);
//             setConnections(createConnections(generatedAgents));
//             setError("Agents created successfully from chat!");
//         } catch (err) {
//             console.error("Error creating agents from chat:", err);
//             setError(err.message || "An unexpected error occurred while creating agents.");
//         }
//     };
...
//                                 <h2 className="text-xl font-semibold text-blue-300">Chat</h2>
//                                 <Button onClick={clearChat} variant="outline" className="bg-red-600 hover:bg-red-700">
...
//                                 <p className="text-gray-300 mb-4">{suggestedPrompt}</p>
//                                 <Button onClick={createAgentsFromChat} className="bg-green-600 hover:bg-green-500">
...
//                                     </div>
//                                     <div>
//                                         <label htmlFor="model" className="block text-sm font-medium text-blue-400 mb-1">
//                                             Model:
//                                         </label>
//                                         <Select
...
//                                     </div>
//                                     <div>
//                                         <label htmlFor="contextLength" className="block text-sm font-medium text-blue-400 mb-1">
//                                             Context Length:
//                                         </label>
//                                         <Input
//                                             id="contextLength"
...
//                                     </div>
//                                     <div>
//                                         <label htmlFor="ethics" className="block text-sm font-medium text-blue-400 mb-1">
//                                             Ethics Level: {ethics}
//                                         </label>
//                                         <Slider
//                                             id="ethics"
...
//                                             step={1}
//                                             value={[ethics]}
//                                             onValueChange={(value) => setEthics(value[0])}
//                                             className="w-full"
//                                         />
//                                     </div>
//                                     <div>
//                                         <label htmlFor="morality" className="block text-sm font-medium text-blue-400 mb-1">
//                                             Morality Level: {morality}
//                                         </label>
//                                         <Slider
//                                             id="morality"
//                                             min={0}
...
//                                             step={1}
//                                             value={[morality]}
//                                             onValueChange={(value) => setMorality(value[0])}
//                                             className="w-full"
//                                         />
//                                     </div>
//                                     <div>
//                                         <label htmlFor="splitPropensity" className="block text-sm font-medium text-blue-400 mb-1">
//                                             Split Propensity: {splitPropensity}
//                                         </label>
//                                         <Slider
//                                             id="splitPropensity"
//                                             min={0}
//                                             max={100}
//                                             step={1}
//                                             value={[splitPropensity]}
//                                             onValueChange={(value) => setSplitPropensity(value[0])}
//                                             className="w-full"
//                                         />
//                                     </div>
//                                     <div>
