...
import { ChevronDown, ChevronRight, Folder, Sparkles, Brain, Clock, Zap, Eye, Database, Code, Cloud, Settings, Plus, ArrowRight, XCircle, Pause, Play, SkipForward, Save, Upload, Download, Trash2, RefreshCw } from 'lucide-react';
...
const CosmicNexusOrchestrator = () => {
    ...
    const runCosmicWorkflow = async () => {
        ...
        try {
            ...
            for (const agent of generatedAgents) {
                ...
                dispatch({ type: 'SET_THINKING_LOG', payload: [...state.thinkingLog, `${agent.name} is processing the cosmic energies...`] });
                ...
                dispatch({ type: 'SET_THINKING_LOG', payload: [...state.thinkingLog, `${agent.name} finished in ${timeTaken} cosmic seconds.`] });
    ...
    const CosmicFileTree = ({ data, onSelect, selectedFiles }) => {
        ...
        const FileItem = ({ path, name }) => {
            ...
            return (
                <div
                    ref={drag}
                    className={`flex items-center space-x-2 py-1 ${selectedFiles.includes(path) ? 'bg-purple-100 rounded' : ''} ${isDragging ? 'opacity-50' : ''}`}
                >
                    <Checkbox
                    ...
                    <span className="cursor-pointer hover:text-purple-600">{name}</span>
                </div>
            );
        };
        ...
        const FolderItem = ({ path, name, children }) => {
            ...
            return (
                <div ref={drop} className={isOver ? 'bg-purple-100' : ''}>
                    <div
                        className="flex items-center space-x-2 py-1 cursor-pointer hover:bg-purple-50"
                        onClick={() => toggleExpand(path)}
                    >
                        {expanded[path] ? <ChevronDown className="w-4 h-4 text-purple-500" /> : <ChevronRight className="w-4 h-4 text-purple-500" />}
                            ...
                            <Card className="bg-gray-800">
                                ...
                                <CardContent>
                                    ...
                                    {state.workflowOutput && (
                                        <div>
                                            <h3 className="text-lg font-semibold mb-2 text-purple-300">Cosmic Workflow Output:</h3>
                                            <ScrollArea className="h-40 bg-gray-700 p-2 rounded-lg">
                                                <ReactMarkdown
                                                    components={{
