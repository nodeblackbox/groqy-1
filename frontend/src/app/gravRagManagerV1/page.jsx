...
import { ChevronDown, ChevronRight, File, Folder, Search, Settings, Code, Database, Brain, Play, Plus, Trash2, Save, RefreshCw, ArrowUpRight } from 'lucide-react'
...
export default function EnhancedComprehensiveUI() {
    ...
    const fetchProjectStructure = async () => {
        try {
            ...
            dispatch({ type: ACTION_TYPES.SET_PROJECT_STRUCTURE, payload: data.structure })
        } catch (error) {
    ...
    const fetchCollections = async () => {
        try {
            ...
            const data = await response.json()
            dispatch({ type: ACTION_TYPES.SET_COLLECTIONS, payload: data.collections })
        } catch (error) {
    ...
    const handleFileSelect = async (filePath) => {
        ...
        try {
            ...
            const data = await response.json()
            dispatch({ type: ACTION_TYPES.SET_FILE_CONTENT, payload: data.fileContents[filePath] })
        } catch (error) {
    ...
    const handleQuery = async () => {
        ...
        try {
            const response = await fetch('/api/qdrant/query', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ collection: state.selectedCollection, query: state.queryInput }),
            })
            const data = await response.json()
            dispatch({ type: ACTION_TYPES.SET_QUERY_RESULT, payload: JSON.stringify(data, null, 2) })
        } catch (error) {
    ...
    const handleSaveFile = async () => {
        ...
        try {
            const response = await fetch('/api/save-file', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ filePath: state.selectedFile, content: state.fileContent }),
            })
            if (response.ok) {
                toast({
                    title: 'Success',
            ...
            <div className="flex-1 p-4 overflow-auto">
                <Tabs defaultValue="file-content">
                    ...
                    <TabsContent value="file-content">
                        <Card>
                            <CardHeader>
                                <CardTitle>{state.selectedFile || 'No file selected'}</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <Textarea
                                    value={state.fileContent}
                                    onChange={(e) => dispatch({ type: ACTION_TYPES.SET_FILE_CONTENT, payload: e.target.value })}
                                    ...
                                    {state.analysis && (
                                        <Accordion type="single" collapsible className="w-full">
                                            <AccordionItem value="file-info">
                                                ...
                                                <AccordionContent>
                                                    <div className="space-y-2">
                                                        <p><strong>File:</strong> {state.analysis.filePath}</p>
                                                        <p><strong>Size:</strong> {state.analysis.fileSize} bytes</p>
                                                        <p><strong>Last Modified:</strong> {new Date(state.analysis.lastModified).toLocaleString()}</p>
                                                    </div>
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="code-structure">
                                                ...
                                                <AccordionContent>
                                                    <div className="space-y-2">
                                                        <p><strong>Functions:</strong> {state.analysis.functions?.length || 0}</p>
                                                        <p><strong>Classes:</strong> {state.analysis.classes?.length || 0}</p>
                                                        <p><strong>Imports:</strong> {state.analysis.imports?.length || 0}</p>
                                                    </div>
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="complexity">
                                                ...
                                                <AccordionContent>
                                                    <div className="space-y-2">
                                                        <p><strong>Cyclomatic Complexity:</strong> {state.analysis.cyclomaticComplexity}</p>
                                                        <p><strong>Maintainability Index:</strong> {state.analysis.maintainabilityIndex}</p>
                                                    </div>
                                                </AccordionContent>
                                            </AccordionItem>
                                            ...
                                            <AccordionItem value="raw-data">
                                                ...
                                                <AccordionContent>
                                                    <pre className="bg-muted p-2 rounded-md overflow-auto max-h-[300px]">
                                                        <code>{JSON.stringify(state.analysis, null, 2)}</code>
                                                    </pre>
                                                </AccordionContent>
                                            </AccordionItem>
                                        </Accordion>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    ...
                                    >
                                        {state.isProcessing ? 'Generating Insights...' : 'Get LLM Insights'}
                                    </Button>
                                    {state.llmInsights && (
                                        <div className="mt-4 prose max-w-none">
                                            <div dangerouslySetInnerHTML={{ __html: state.llmInsights }} />
                                        </div>
                                    )}
                                </div>
                            </CardContent>
                            <CardFooter>
                                <Button
                                    onClick={() => {/* Implement save insights functionality */ }}
                                    ...
                                    <div>
                                        ...
                                        <div className="flex space-x-2 mb-4">
                                            <Select
                                                ...
                                                onValueChange={(value) => dispatch({ type: ACTION_TYPES.SET_SELECTED_COLLECTION, payload: value })}
                                            >
                                            ...
                                            <Button
                                                variant="destructive"
                                                onClick={() => handleDeleteCollection(state.selectedCollection)}
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-4">
                                            <Textarea
                                                ...
                                                onChange={(e) => dispatch({ type: ACTION_TYPES.SET_QUERY_INPUT, payload: e.target.value })}
                                            />
                                            <Button onClick={handleQuery} disabled={state.isProcessing}>
                    ...
                    <TabsContent value="settings">
                        <Card>
                                ...
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => dispatch({ type: ACTION_TYPES.SET_SETTINGS, payload: { model: value } })}
                                        >
                                    ...
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => dispatch({ type: ACTION_TYPES.SET_SETTINGS, payload: { maxTokens: parseInt(e.target.value) } })}
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <Slider
                                            ...
                                            onValueChange={(value) => dispatch({ type: ACTION_TYPES.SET_SETTINGS, payload: { temperature: value[0] } })}
                                        />
                                    </div>
                                    <div className="flex items-center space-x-2">
                                        <Switch
                                            ...
                                            onCheckedChange={(checked) => dispatch({ type: ACTION_TYPES.SET_SETTINGS, payload: { useCache: checked } })}
                                        />
                            ...
                            <CardFooter>
                                <Button onClick={() => {/* Implement save settings functionality */ }}>
                                    Save Settings
                                </Button>
                            </CardFooter>
                        </Card>
