...
const generateTasksFromPrompt = async (prompt, knowledgeBase, apiKey) => {
    try {
        const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are an AI assistant that generates tasks based on prompts. Generate a list of tasks in JSON format, including potential duplications or variations if appropriate.",
                    },
                    {
                        role: "user",
        ...
        const generatedTasks = JSON.parse(data.choices[0].message.content);
...
export default function EnhancedWorkflowBuilder() {
        ...
        async (prompt) => {
            ...
            for (const task of generatedTasks) {
                dispatch({
                    ...
                    payload: {
                        id: task.id,
                        type: task.type.toLowerCase() + "Agent",
                        label: task.name,
                    },
                });
            }
    ...
    const performMitosis = useCallback(async () => {
        ...
        generateMermaidCode();
    }, [state.mitosisEnabled, state.workflow, generateMermaidCode]);
