...
const TreeNode = ({ node, onAdd, onDelete, onToggle, onEdit, searchTerm }) => {
    ...
    const matchesSearch =
        node?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false;
...
export default function APITestV5Page() {
    ...
    const handleAuthKeysSave = async () => {
        // Auth keys are already persisted via useEffect
    ...
    useEffect(() => {
        const fetchData = async () => {
            const storedDarkMode = await db.jsonStructures
                .where("id")
                .equals(1)
                .first();
            if (storedDarkMode)
                setDarkMode(JSON.parse(storedDarkMode.structure).darkMode);

            const storedJsonStructure = await db.jsonStructures
                .where("id")
                .equals(1)
                .first();
            if (storedJsonStructure)
                setJsonStructure(
                    JSON.parse(storedJsonStructure.structure).jsonStructure
                );
    ...
    useEffect(() => {
        db.jsonStructures.put({
            id: 1,
            structure: JSON.stringify({ jsonStructure }),
        });
    }, [jsonStructure]);
    ...
    useEffect(() => {
        db.apiKeys
            .clear()
            .then(() => {
                const keysToAdd = Object.entries(authKeys).map(([provider, key]) => ({
                    provider,
                    key,
                }));
    ...
    const handleExecuteAI = () => {
        ...
        // Mock AI processing
        const response = `AI Response to "${aiCommand}" with complexity ${complexity}% and ${isRealTime ? "real-time" : "batch"
            } processing.`;
        setAiLogs([
            ...aiLogs,
            { command: aiCommand, response, timestamp: new Date().toLocaleString() },
        ]);
        setChatHistory([
            ...chatHistory,
            {
                userMessage: aiCommand,
                botMessage: response,
    ...
    const handleSaveTemplate = async (templateName) => {
        ...
        setAiTemplates([
            ...aiTemplates,
            { name: templateName, command: aiCommand, complexity, isRealTime },
        ]);
    ...
    // Example: If message includes "fetch apis", list available APIs
        ...
        {
            response += `Here are the available APIs: ${apiEndpoints
                .map((ep) => ep.name)
                .join(", ")}.`;
        } else if (message.toLowerCase().includes("virtual database"))
        {
            response += `Our virtual database includes the following tables: ${databases
                .map((db) => db.tables.join(", "))
                .flat()
                .join(", ")}.`;
        } else
        {
            response +=
                "I am here to assist you with API testing and database interactions.";
        }
        setChatHistory([
            ...chatHistory,
            {
    ...
    return (
        <div
            className={`flex h-screen ${darkMode ? "bg-gray-900" : "bg-white"
        ...
        >
            ...
            {showAPIManager ? (
                ...
                <div
                        className={`flex-1 flex flex-col p-8 overflow-auto ${darkMode ? "bg-gray-900" : "bg-gray-100"
                            } text-white`}
                    >
                        ...
                        >
                            ...
                            <TabsContent value="database" className="space-y-4">
                                <QdrantManager
                                    // Ensure jsonStructure is defined and contains a collection property
                                    collection={(jsonStructure && jsonStructure.collection) || "default"}
                                    setCollection={(col) =>
                                        setJsonStructure({
                                            ...jsonStructure,
                                            collection: col,
                                    ...
                                    <ul className="space-y-2">
                                        {databases.map((dbItem) => (
                                            <li
                                                ...
                                                onClick={() => selectDatabase(dbItem)}
                                            >
                                                <div>
                                                    <p className="font-semibold">{dbItem.name}</p>
                                                    <p className="text-sm">
                                                        {dbItem.type} - {dbItem.host}:{dbItem.port}
                                                    </p>
                                                </div>
                                                <Button
                                                    variant="ghost"
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        deleteDatabaseHandler(dbItem.id);
                                                    }}
                                                >
                                                    <Trash size={14} />
                                                </Button>
                                ...
                                <div className="bg-black p-6 rounded-xl">
                                    <div className="flex justify-between items-center mb-4">
                                        ...
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={suggestQueryOptimization}
                                        >
                                            Optimize Query
                                        </Button>
                                    </div>
                                        ...
                                        <div className="bg-gray-800 p-4 rounded-lg">
                                            ...
                                            <pre className="text-sm">
                                                {Array.isArray(queryResult)
                                                    ? JSON.stringify(getPaginatedResults(), null, 2)
                                                    : queryResult}
                                            </pre>
                                            {Array.isArray(queryResult) && (
                                                <div className="flex justify-between items-center mt-2">
                                                    ...
                                                    <div className="flex space-x-2">
                                                        <Button
                                                            ...
                                                            size="sm"
                                                            onClick={() =>
                                                                setQueryPage((prev) => Math.max(prev - 1, 1))
                                                            }
                                                        ...
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            onClick={() => setQueryPage((prev) => prev + 1)}
                                                            disabled={
                                                                queryPage * queryPageSize >= queryResult.length
                                                            }
                                                        >
                                                            Next
                                                        </Button>
                                ...
                                <div className="bg-black p-6 rounded-xl">
                                    ...
                                    <div className="h-64 flex items-center justify-center bg-gray-800 rounded-lg">
                                        <span className="text-gray-400">
                                            Interactive data visualizations would be displayed here
                                        </span>
                                    </div>
                                </div>
                            </TabsContent>
            ...
            <Dialog open={isAuthDialogOpen} onOpenChange={setIsAuthDialogOpen}>
                <DialogContent>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <Input
                                        ...
                                        onChange={(e) =>
                                            setAuthKeys({ ...authKeys, openai: e.target.value })
                                        }
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <Input
                                        ...
                                        onChange={(e) =>
                                            setAuthKeys({ ...authKeys, anthropic: e.target.value })
                                        }
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <Input
                                        ...
                                        value={authKeys.groq}
                                        onChange={(e) =>
                                            setAuthKeys({ ...authKeys, groq: e.target.value })
                                        }
                                    ...
                                    <Button
                                        type="button"
                                        onClick={() =>
                                            setAuthKeys((prev) => ({ ...prev, groq: authKeys.groq }))
                                        }
                                        className="absolute inset-y-0 right-0 flex items-center px-4 text-gray-400 hover:text-white"
                                    >
                    ...
                    <DialogFooter>
                        <Button onClick={handleAuthKeysSave}>Save</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
