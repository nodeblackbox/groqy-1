...
export default function EnhancedACEWorkflowBuilder() {
    ...
    const runWorkflow = async () => {
        ...
        try {
            ...
            const generatedAgents = await agentFactory.createAgentsFromPrompt(workflowInput);
            ...
            for (const agent of generatedAgents) {
                ...
                setWorkflowOutput((prevOutput) => prevOutput + `### ${agent.name}\n**Input:**\n${currentInput}\n\n**Delegation:**\n${delegation}\n\n**Output:**\n${response}\n\n*Time taken: ${timeTaken} seconds*\n\n`);
                ...
                {workflowOutput && (
                    <div>
                        ...
                        <ScrollArea className="h-96 bg-gray-800 p-4 rounded-lg border border-blue-500">
                            <pre className="text-green-300 whitespace-pre-wrap">{workflowOutput}</pre>
                        </ScrollArea>
                    </div>
                )}
            </div>
