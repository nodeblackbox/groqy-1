...
// ACEAgent Class
class ACEAgent {
    constructor(id, name, knowledgeBase, model, contextLength, ethics, morality) {
        this.id = id;
        this.name = name;
...
class AgentFactory {
    ...
    async createAgentsFromPrompt(prompt, model, contextLength, ethics, morality, maxRetries = 3) {
        let retries = 0;
    ...
    "inputOutput": {
      ...
      "api": {}
                ...
                return generatedAgents.map(agentData => {
                    const agent = new ACEAgent(generateUniqueId(), agentData.name, this.knowledgeBase, model, contextLength, ethics, morality);
...
export default function EnhancedACEWorkflowBuilder() {
    ...
    const addAgent = () => {
        const newAgent = new ACEAgent(generateUniqueId(), `Agent ${agents.length + 1}`, knowledgeBase, selectedModel.id, contextLength, ethics, morality);
    ...
    const runWorkflow = async () => {
        ...
        try {
            ...
            const generatedAgents = await agentFactory.createAgentsFromPrompt(workflow.input, selectedModel.id, contextLength, ethics, morality);
            ...
            for (const agent of generatedAgents) {
                ...
                if (response.length > 500) { // Arbitrary threshold for demonstration
                    const newAgent = new ACEAgent(generateUniqueId(), `${agent.name} Clone`, knowledgeBase, selectedModel.id, contextLength, ethics, morality);
    ...
    const createAgentsFromChat = async () => {
        ...
        try {
            const agentFactory = new AgentFactory(apiKey, knowledgeBase);
            const generatedAgents = await agentFactory.createAgentsFromPrompt(suggestedPrompt, selectedModel.id, contextLength, ethics, morality);
