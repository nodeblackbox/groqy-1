                ...
                <div className="mb-4">
                    ...
                    {subtasks.map((subtask, index) => (
                        <div key={subtask.id} className="mb-2 p-2 bg-gray-700 rounded">
                            ...
                            {subtask.type === "API" && (
                                <p className="text-blue-300">URL: {subtask.config.url}</p>
                            )}
...
const TaskFormModal = () => {
    const { state, saveTask, dispatch, agents, delegateTaskToAgent } =
        useWorkflow();
...
const evaluateCondition = async (conditionExpression) => {
    try {
        // Use a safe evaluation library or write a parser
...
const executeLogicSafely = async (logicCode) => {
    try {
        // Use a safe JavaScript sandbox or interpreter
...
const generatePage = async (path) => {
    // Implement your page generation logic here
    await new Promise((resolve) =>
