...
// Styles
import styles from "../../styles/glitchnav.module.css";
...
const NavBar = ({ onOpenSettings, onOpenBugTracker, onOpenKanban }) => {
  return (
    <nav className="bg-gradient-to-r from-gray-900 via-blue-900 to-gray-900 p-6 flex justify-between items-center mb-4 rounded-lg shadow-lg">
      <div className="glitch-container relative">
        <div
          className="glitch text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 animate-pulse"
          data-text="GROQY"
        >
          GROQY
        </div>
      ...
      <h1 className="text-3xl font-bold text-center text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600 flex items-center justify-center">
        AI VENTURE STUDIO
      ...
      <div className="flex space-x-4">
        {[
          ...
          { label: "Bug Tracker", icon: Bug, onClick: onOpenBugTracker },
          { label: "Kanban", icon: Trello, onClick: onOpenKanban },
        ].map((item) => (
          <button
            key={item.label}
            onClick={item.onClick}
            className="group bg-gray-800 hover:bg-gray-700 text-cyan-400 px-4 py-2 rounded-full flex items-center transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-cyan-500/50"
          >
            <item.icon size={16} className="mr-2 group-hover:animate-spin" />
        ...
        <AnimatePresence>
          {Object.keys(files).map((filePath) => (
            <motion.div
              ...
              className={`px-4 py-2 flex items-center cursor-pointer transition-colors duration-200 ${
                activeFile === filePath
      ...
      <div className="p-4 border-t border-gray-800 bg-gray-900">
        <Tab.Group>
          <Tab.List className="flex space-x-1 rounded-xl bg-gray-800 p-1 mb-4">
            <Tab
              className={({ selected }) =>
                `w-full rounded-lg py-2.5 text-sm font-medium leading-5 text-cyan-400 ring-white ring-opacity-60 ring-offset-2 ring-offset-cyan-400 focus:outline-none focus:ring-2 ${
                  selected
                    ? "bg-gray-900 shadow"
            ...
            <Tab.Panel>
              ...
              <div className="flex mb-4">
                ...
                <motion.button
                  ...
                  className={`bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-full flex items-center transition-colors duration-200 shadow-lg ${
                    isGenerating || !apiKey
...
const LinuxIDE = () => {
  ...
  const handleSelectFile = async (filePath) => {
    // Ensure the filePath is correctly quoted to handle spaces
    const safeFilePath = `"${filePath}"`; // Enclose the file path in quotes

    if (!openFiles[filePath]) {
      try {
        // Use the safeFilePath in your command
        const content = await executeCommand(`cat src/${safeFilePath}`);
  ...
  const handleSaveFile = async (filePath, content) => {
    try {
      ...
      // Escape single quotes in content for proper command execution
      const escapedContent = content.replace(/'/g, "'\\''");

      // Use the safeFilePath in your command
      await executeCommand(`echo '${escapedContent}' > src/${safeFilePath}`);
    ...
    return (
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-56">
          ...
          <DropdownMenuItem>
            <span className="font-semibold">Ctrl + B:</span> Toggle file
            explorer
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
  ...
  const TerminalSection = ({
    ...
    return (
      ...
      >
        <div className="flex justify-between items-center p-2 bg-gray-800">
          <div className="flex items-center">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onSelect={() => setActiveTab("terminal")}>
                  Terminal
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={() => setActiveTab("output")}>
                  Output
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={() => setActiveTab("debug")}>
                  Debug
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        {!isCollapsed && (
          <ScrollArea className="h-48">{tabContent[activeTab]}</ScrollArea>
        )}
      </NeomorphicContainer>
    );
  };
